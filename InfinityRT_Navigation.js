...
infinityrt_navigation = function (scene, w, h) {
    ...
    this._navMode = 2;
//    this._navMinDolly = -20.0;
//    this._navMaxDolly = 145.0;
    this._navMinDolly = 120.0;
    this._navMaxDolly = 1010.0;
...
//    this._navMaxDolly = 100.0;
    ...
    this._navPanDolly = -175.0; // Distance when pan changes to rotation
...
//    this._panMax = [45, 17]; //[right, top];
//    this._panMin = [-45, -13]; //[left, bottom];
//    this._panMax = [99, 99];    //[left, bottom];
//    this._panMin = [-99, -99];  //[right, top];
    this._panMax = [55, 20];    //[left, bottom];
    this._panMin = [-55, -7];  //[right, top];
}
...
infinityrt_navigation.prototype.NavStartGotoPos = function (yang, xang, xpan, ypan, dolly, numFrames, onComplete, onSample) {
    ...
    if (this._navGotoPosEnableSlowInOut || this.onSample != null) {
        ...
        if (isNaN(xpan))
            ...
            this._navGotoPosDelta.navXPan = {
                t0: this._navPan[0],
                td: (xpan - this._navPan[0])
            };
        if (isNaN(ypan))
            this._navGotoPosDelta.navYPan = {
                t0: this._navPan[1],
                td: 0
            };
        else
            this._navGotoPosDelta.navYPan = {
                t0: this._navPan[1],
                td: (ypan - this._navPan[1])
            };
        // Zoom
        this._navGotoPosDelta.navDolly = {
            t0: this._navDolly,
            td: (dolly - this._navDolly)
        };
    } else {
        // Rotation
    ...
    if (this._navMode == 0 || this._navMode == 2) {
        if (this._navMode == 2) {
            ...
            if (this._navDXAng != 0.0 || this._navDYAng != 0.0 || this._navDPan[0] != 0.0 || this._navDPan[1] != 0.0 || this._navDDolly != 0.0) {
               ...
               // var fRotMinLimit = -dollyfrac * (M_PI * -0.012045586716505384);
...
infinityrt_navigation.prototype.NavRotation = function (mpos, mdelta) {
    ...
    if (this._navMode == 2) {
        ...
        this._navMX = mpos[0];
        this._navMY = mpos[1];
        this._navDX = -mdelta[0];
        this._navDY = -mdelta[1];
    } else {
        this._navXAng -= mdelta[1] / 30.0;
        this._navYAng += mdelta[0] / 30.0;
        var fRotLimit = M_PI * 0.48;
        //        var fRotMinLimit = -M_PI * 0.08;
        ///var fRotMinLimit = -dollyfrac * (M_PI * -0.01245586716505384);
        var fRotMinLimit = -dollyfrac * (M_PI * -0.001);
        if (this._navXAng > fRotLimit)
            this._navXAng = fRotLimit;
        else if (this._navXAng < fRotMinLimit)
